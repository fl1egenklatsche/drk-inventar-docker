                ...
                <div class="product-item">
                    ...
                    <label>
                        <input type="checkbox" id="missing_${productId}" ${isCompletelyMissing ? 'checked' : ''}
                               onclick="toggleInstancesVisibility('${productId}', this.checked)"> 
                        Fehlt komplett
                    </label>
...
function setupCurrentCompartmentEvents() {
    ...
    // Event-Delegation verwenden - lausche auf Änderungen im ganzen inspection-content
    const inspectionContent = document.getElementById('inspection-content');
    if (!inspectionContent) {
        console.error('inspection-content not found!');
        return;
    }
    
    // Entferne alte Event-Listener um Duplikate zu vermeiden
    if (inspectionContent._checkboxHandler) {
        inspectionContent.removeEventListener('change', inspectionContent._checkboxHandler);
    }
    ...
    const checkboxHandler = function(event) {
        ...
        // Prüfe ob es eine "missing" Checkbox ist
        if (target.type === 'checkbox' && target.id.startsWith('missing_')) {
            console.log('=== CHECKBOX CLICKED ===');
            console.log('Checkbox ID:', target.id);
            console.log('Checkbox checked:', target.checked);
            
            // Finde das entsprechende instances div
            const productItem = target.closest('.product-item');
            const instancesDiv = productItem ? productItem.querySelector('.instances') : null;
            
            console.log('Product item found:', !!productItem);
            console.log('Instances div found:', !!instancesDiv);
            ...
            if (instancesDiv) {
                if (target.checked) {
                    ...
                    console.log('✓ Hidden instances for', target.id);
                } else {
                    instancesDiv.style.display = 'block';
                    console.log('✓ Showed instances for', target.id);
                }
            } else {
                console.error('Could not find instances div for', target.id);
            }
        }
    };
    
    // Event-Handler speichern und hinzufügen
    inspectionContent._checkboxHandler = checkboxHandler;
    inspectionContent.addEventListener('change', checkboxHandler);
    
    console.log('✓ Event delegation set up on inspection-content');
    
    // Debug: Alle Checkboxen finden und loggen
    const allCheckboxes = inspectionContent.querySelectorAll('input[type="checkbox"]');
    console.log('Found checkboxes:', allCheckboxes.length);
    allCheckboxes.forEach((cb, idx) => {
        console.log(`Checkbox ${idx}: ID=${cb.id}, checked=${cb.checked}`);
        
        // Test: onclick direkt setzen als Fallback
        cb.onclick = function() {
            console.log('DIRECT CLICK on', this.id, 'checked:', this.checked);
        };
    });
...
// Direkte Toggle-Funktion als Fallback
function toggleInstancesVisibility(productId, isChecked) {
    ...
    if (instancesDiv) {
        if (isChecked) {
            instancesDiv.style.display = 'none';
            // Alle Datumseingaben leeren
            const dateInputs = instancesDiv.querySelectorAll('input[type="date"]');
            dateInputs.forEach(input => input.value = '');
            console.log('✓ DIRECT: Hidden instances for', productId);
        } else {
            instancesDiv.style.display = 'block';
            console.log('✓ DIRECT: Showed instances for', productId);
        }
    } else {
        console.error('DIRECT: Could not find instances div for', productId);
    }
}
